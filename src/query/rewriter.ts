...
const FROM_TABLE_RE = /\bfrom\s+[`"]?(\w+)[`"]?/i;
...
export async function rewriteToAggregate(
  ...
  const errorExpr = await buildErrorCountExpr(sql, instance, org);
...
function extractFromClause(sql: string, fromIdx: number): string {
  ...
  const terminators = [
    after.search(WHERE_RE),
    after.search(ORDER_BY_RE),
    after.search(LIMIT_RE),
    after.search(GROUP_BY_RE),
  ].filter((i) => i > 0);
...
async function buildErrorCountExpr(
  sql: string,
  instance: O2Instance,
  org?: string,
): Promise<string> {
  ...
  const stream = extractStreamName(sql);
  if (stream) {
    const fields = await instance.getStreamFields(stream, org);
    const levelField = LEVEL_FIELD_CANDIDATES.find((f) => fields.includes(f));
    if (levelField) {
      return `COUNT(CASE WHEN ${levelField} = 'error' OR ${levelField} = 'ERROR' THEN 1 END)`;
    }
  }
...
function extractStreamName(sql: string): string | null {
  const m = sql.match(FROM_TABLE_RE);
  return m ? m[1] : null;
}
